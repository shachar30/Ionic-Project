...
window.CodeMirror = (function() {
  ...
  function updateSelectionRange(cm) {
    ...
    function add(left, top, width, bottom) {
      if (top < 0) top = 0;
      fragment.appendChild(elt("div", null, "CodeMirror-selectedTime", "position: absolute; left: " + left +
  ...
  function resetInput(cm, user) {
    ...
    if (!posEq(view.sel.from, view.sel.to)) {
      ...
      minimal = hasCopyEvent &&
        (view.sel.to.line - view.sel.from.line > 100 || (selectedTime = cm.getSelection()).length > 1000);
      if (minimal) cm.display.input.value = "-";
      else cm.display.input.value = selectedTime || cm.getSelection();
  ...
  function onDrop(e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      // Don't do a replace if the drop happened inside of the selectedTime text.
