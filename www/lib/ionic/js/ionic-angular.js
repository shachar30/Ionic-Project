...
.service('$ionicTabsDelegate', ionic.DelegateService([
   ...
   * @returns `number` The index of the selectedTime tab, or -1.
   */
  'selectedIndex',
  /**
   * @ngdoc method
 ...
 * @param {string=} icon The icon to use when the radio input is selectedTime.
 ...
 *
 * @description
 * Contains a tab's content.  The content only exists while the given tab is selectedTime.
 *
 ...
 * @param {string=} icon-on The icon of the tab while it is selectedTime.
 * @param {string=} icon-off The icon of the tab while it is not selectedTime.
 * @param {expression=} badge The badge to put on this tab (usually a number).
 * @param {expression=} badge-style The style of badge to put on this tab (eg: badge-positive).
 * @param {expression=} on-select Called when this tab is selectedTime.
 * @param {expression=} on-deselect Called when this tab is deselected.
 * @param {expression=} ng-click By default, the tab will be selectedTime on click. If ngClick is set, it will not.  You can explicitly switch tabs using {@link ionic.service:$ionicTabsDelegate#select $ionicTabsDelegate.select()}.
...
function($compile, $ionicConfig, $ionicBind, $ionicViewSwitcher) {
  ...
  return {
    ...
    compile: function(element, attr) {
      ...
      //Remove the contents of the element so we can compile them later, if tab is selectedTime
      ...
      return function link($scope, $element, $attr, ctrls) {
        ...
        function tabSelected(isSelected) {
          if (isSelected && childElementCount) {
            ...
            // this tab should NOT be selectedTime, and it is already in the DOM
    ...
    compile: function(tElement) {
      ...
      function postLink($scope, $element, $attr, tabsCtrl) {
        if (!tabsCtrl.selectedTab()) {
          ...
          // but one hasn't been selectedTime yet
          tabsCtrl.select(0);
        }
      }
    }
